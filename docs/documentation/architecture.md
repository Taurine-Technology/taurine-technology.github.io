...
The example network highlights the services running on each device. The network below is equivalent to the network configured in the [basic examples](../examples/basic-setup.md).
...
The server needs to be able to connect to the switches, so it must be on the same LAN as the switches.
...
This collects system resources statistics and sends them to the backend for historical and real time monitoring.

#### Port Monitor

This monitors the ports assoscited with a switch's bridge and sends throughput data to the backend for historical and real time monitoring
...
This tool collects data such as the number of bytes processed by a flow table entry, this is sent to the backend and linked to classification data to monitor the amount of data used per application.
